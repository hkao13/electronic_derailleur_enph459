...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        in_1_PC =   (in_1_PC & \
                                (uint32)(~(uint32)(in_1_DRIVE_MODE_IND_MASK << (in_1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (in_1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(in_1_DR & (uint8)(~in_1_MASK));
    drVal = (drVal | ((uint8)(value << in_1_SHIFT) & in_1_MASK));
    in_1_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(in_1__0__SHIFT, mode);
}
...
{
    return (uint8)((in_1_PS & in_1_MASK) >> in_1_SHIFT);
}
...
{
    return (uint8)((in_1_DR & in_1_MASK) >> in_1_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(in_1_INTSTAT & in_1_MASK);
		in_1_INTSTAT = maskedStatus;
        return maskedStatus >> in_1_SHIFT;
    }
