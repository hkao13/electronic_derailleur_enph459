...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        in_2_PC =   (in_2_PC & \
                                (uint32)(~(uint32)(in_2_DRIVE_MODE_IND_MASK << (in_2_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (in_2_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(in_2_DR & (uint8)(~in_2_MASK));
    drVal = (drVal | ((uint8)(value << in_2_SHIFT) & in_2_MASK));
    in_2_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(in_2__0__SHIFT, mode);
}
...
{
    return (uint8)((in_2_PS & in_2_MASK) >> in_2_SHIFT);
}
...
{
    return (uint8)((in_2_DR & in_2_MASK) >> in_2_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(in_2_INTSTAT & in_2_MASK);
		in_2_INTSTAT = maskedStatus;
        return maskedStatus >> in_2_SHIFT;
    }
