...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        in_4_PC =   (in_4_PC & \
                                (uint32)(~(uint32)(in_4_DRIVE_MODE_IND_MASK << (in_4_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (in_4_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(in_4_DR & (uint8)(~in_4_MASK));
    drVal = (drVal | ((uint8)(value << in_4_SHIFT) & in_4_MASK));
    in_4_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(in_4__0__SHIFT, mode);
}
...
{
    return (uint8)((in_4_PS & in_4_MASK) >> in_4_SHIFT);
}
...
{
    return (uint8)((in_4_DR & in_4_MASK) >> in_4_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(in_4_INTSTAT & in_4_MASK);
		in_4_INTSTAT = maskedStatus;
        return maskedStatus >> in_4_SHIFT;
    }
